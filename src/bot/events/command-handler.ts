...
const Logs = require('../../../config/config.json');
...
export class CommandHandler implements EventHandler {
	...
	public async process(intr: CommandInteraction | AutocompleteInteraction): Promise<void> {
		...
		const command = CommandUtils.findCommand(this.commands, commandParts.filter(Boolean) as string[]);
		if (!command) {
			Logger.error(
	...
	private sendError(intr: CommandInteraction, data: EventData) {
