		...
		try {
			if (!Config.developers.includes(intr.user.id)) {
				await InteractionUtils.send(
					intr,
					Lang.getEmbed('validationEmbeds.devOnly', data.lang).unwrap()
				);
				return;
			}
					...
					if (intr.client.shard) {
						try {
							...
							if (error instanceof Error && error.name.includes('ShardingInProcess')) {
								await InteractionUtils.send(
									intr,
									Lang.getEmbed('errorEmbeds.startupInProcess', data.lang).unwrap()
								);
								return;
							} else {
								throw error;
			...
			if (error instanceof Error) {
				...
				Logger.error(
					'An unknown error occurred while executing the dev command.' +
						`\nInteraction ID: ${intr.id}\nUser ID: ${intr.user.id}`
				);
			}
		}
